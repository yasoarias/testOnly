<template>
  <div class="package-section">
    <Navbar
      ...
      @logout="handleLogout"
    />
    <h2 class="section-title">Premium Kiddie Party Event Packages</h2>
    ...
    <div v-else class="packages-container">
      <div class="packages-wrapper">
        <div class="packages-row">
          <div
            v-for="product in kiddiePartyProducts"
            :key="product.id"
            class="package-card"
          >
            <img
...
const fetchKiddiePartyProducts = async () => {
  isLoading.value = true;
  error.value = null;
  try {
    const response = await fetch(
      "http://localhost:3000/api/products?category=kiddieparty",
      {
        headers: {
          Authorization: `Bearer ${token.value}`,
        },
    ...
    if (response.ok) {
      ...
      kiddiePartyProducts.value = data.filter(
        (product) => product.category.toLowerCase() === "kiddieparty"
      );
    } else {
      throw new Error("Failed to fetch kiddie party products");
    }
  } catch (err) {
    console.error("Error fetching kiddie party products:", err);
    error.value =
      "Failed to load kiddie party packages. Please try again later.";
  } finally {
